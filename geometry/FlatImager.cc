...
FlatImager::FlatImager(TsParameterManager* pM, TsExtensionManager* eM, TsMaterialManager* mM, TsGeometryManager* gM,
...
TsVGeometryComponent::SurfaceType FlatImager::GetSurfaceID(G4String surfaceName) {
    ...
    G4cout << "----------------------" << G4endl;
    G4cout << "GetSurfaceID: " << surfaceNameLower << G4endl;
    G4cout << "----------------------" << G4endl;
    ...
    else if (surfaceNameLower == "photodetectorsurface")
        surfaceID = ZMinusSurface; // Surface BEHIND the detector
    else if (surfaceNameLower == "photodetectorscintillatorinterface")
        surfaceID = ZPlusSurface;   // Interface between Photodetector and scintillator
...
G4bool FlatImager::IsOnBoundary(G4ThreeVector localpos, G4VSolid* solid, SurfaceType surfaceID) {
    ...
    default:
        ...
        G4cerr << "FlatImager::IsOnBoundary called for unknown surface of component: " << fName << G4endl;
        fPm->AbortSession(1);
        return false;
    }
    G4cout << "==============================" << G4endl;
...
G4double FlatImager::GetAreaOfSelectedSurface(G4VSolid* solid, SurfaceType surfaceID, G4int, G4int, G4int) {
...
void FlatImager::NoteIndexError(
    ...
    G4cerr << "returned " << coordinate << " index: " << value << " outside of the valid range of 0 to " << limit
           << G4endl;
...
void FlatImager::NoteParameterizationError(G4double energy, G4String componentName, G4String volumeName) {
#ifdef TOPAS_MT
...
G4int FlatImager::GetIndex(G4Step* aStep) {
    ...
    if (fPtDtDivisionCount[0] * fPtDtDivisionCount[1] * fPtDtDivisionCount[2] == 1) {
        G4cout<<"FlatImager::GetIndex"<<G4endl;
        ...
        if (iX < 0 || iX >= fPtDtDivisionCount[0]) {
            NoteIndexError(aStep->GetTotalEnergyDeposit(), GetName(), "X", iX, fPtDtDivisionCount[0] - 1);
            return -1;
        }

        if (iY < 0 || iY >= fPtDtDivisionCount[1]) {
            NoteIndexError(aStep->GetTotalEnergyDeposit(), GetName(), "Y", iY, fPtDtDivisionCount[1] - 1);
            return -1;
        }

        if (iZ < 0 || iZ >= fPtDtDivisionCount[2]) {
            NoteIndexError(aStep->GetTotalEnergyDeposit(), GetName(), "Z", iZ, fPtDtDivisionCount[2] - 1);
            return -1;
        }
...
G4int FlatImager::GetBin(G4int index, G4int iBin) {
    ...
    G4int binY = int((index - binX * fPtDtDivisionCount[1] * fPtDtDivisionCount[2]) / fPtDtDivisionCount[2]);
    if (iBin == 1) return binY;

    G4int binZ = index - binX * fPtDtDivisionCount[1] * fPtDtDivisionCount[2] - binY * fPtDtDivisionCount[2];
...
void FlatImager::GetCrystalReflectorThickness(void) {
    fCrystalReflectorThickness = fPm->ParameterExists(GetFullParmName("Crystal/ReflectorThickness"))
...
void FlatImager::GetPrefilters(void) {
    fNbPrefilters = fPm->ParameterExists(GetFullParmName("Prefilters/Materials"))
                        ...
                        : 0;
    if (fNbPrefilters > 0) {
        fPrefilterThicknesses = fPm->GetDoubleVector(GetFullParmName("Prefilters/Thicknesses"), "Length");
    }
...
G4double FlatImager::GetCrystalGapHLY(void) {
    if (NbOfYBins == 1) {
        if (std::abs(fHLY - fCrystalHLY) > eps) {
            G4cerr << "Error: if the number of YBins is 1 then HLY should be equal "
                      "to Crystal/HLY, see: "
    ...
    if ((fCrystalHLY + fCrystalReflectorThickness) * NbOfYBins > fHLY) {
        G4cout << "fHLY: " << fHLY << " size: " << (fCrystalHLY + fCrystalReflectorThickness) * NbOfYBins << G4endl;
        G4cerr << "Error: Crystal/HLY * Crystal/NbOfYBins should be smaller than HLY, see: "
...
void FlatImager::ConstructPrefilters(G4VPhysicalVolume* fEnvelopePhys, G4RotationMatrix* RotMatrix) {
    G4VSolid**        PrefilterSolid = new G4VSolid*[fNbPrefilters];
    G4LogicalVolume** PrefilterLV    = new G4LogicalVolume*[fNbPrefilters];
    G4ThreeVector*    TransVectorPrefilter =
        new G4ThreeVector(0, 0, fTotalPrefilterThicknesses * 0.5 + fCrystalHLZ + fPhotoDetectorHLZ - fCollimatorHL);
    ...
    G4cout << TransVectorPrefilter->x() << " " << TransVectorPrefilter->y() << " " << TransVectorPrefilter->z() << " "
           << fTotalPrefilterThicknesses << G4endl;
    if (fNbPrefilters > 0) {
        for (G4int prefilter_ind = 0; prefilter_ind < fNbPrefilters; prefilter_ind++) {
            ...
            TransVectorPrefilter->setZ(
                TransVectorPrefilter->z() - previous_hl - fPrefilterThicknesses[prefilter_ind] * 0.5);
            ...
            PrefilterLV[prefilter_ind] =
                CreateLogicalVolume(LVName, fPrefilterMaterials[prefilter_ind], PrefilterSolid[prefilter_ind]);
...
void FlatImager::ConstructPhotodetector(G4VPhysicalVolume* fEnvelopePhys, G4RotationMatrix* RotMatrix) {

    G4VSolid*        PhotoDetectorSolid = new G4Box("PtDtSd", fHLX, fHLY, fPhotoDetectorHLZ);
    G4LogicalVolume* PhotoDetectorLV    = CreateLogicalVolume("PtDtLV", fPhotoDetectorMaterial, PhotoDetectorSolid);
    G4OpticalSurface* OpSkinSurface1 = new G4OpticalSurface("SkinSurface_photodetector");
    OpSkinSurface1->SetModel(unified);
    OpSkinSurface1->SetType(dielectric_dielectric);
    OpSkinSurface1->SetFinish(polished);

    G4Material* mat = PhotoDetectorLV->GetMaterial();

    G4ThreeVector* TransVectorPhotoDetector =
        new G4ThreeVector(0, 0, 0 - fCollimatorHL - fTotalPrefilterThicknesses * 0.5 - fCrystalHLZ);

    // SetLogicalVolumeToBeSensitive(PhotoDetectorLV);
    ColorLV(G4Colour::Green(), PhotoDetectorLV);
    auto* PDPV = CreatePhysicalVolume("PhDtPV", PhotoDetectorLV, RotMatrix, TransVectorPhotoDetector, fEnvelopePhys);
    ...
    G4VSolid*          XDivisionSolid = new G4Box(divisionName, deltaX, PtDtWidths[1] / 2., PtDtWidths[2] / 2.);
    G4LogicalVolume*   XDivisionLog   = CreateLogicalVolume(divisionName, fPhotoDetectorMaterial, XDivisionSolid);
    G4VPhysicalVolume* XDivisionPhys =
        CreatePhysicalVolume(divisionName, XDivisionLog, PDPV, kXAxis, fPtDtDivisionCount[0], deltaX * 2.);
    ...
    G4LogicalVolume*   YDivisionLog   = CreateLogicalVolume(divisionName, fPhotoDetectorMaterial, YDivisionSolid);
    G4VPhysicalVolume* YDivisionPhys =
        CreatePhysicalVolume(divisionName, YDivisionLog, XDivisionPhys, kYAxis, fPtDtDivisionCount[1], deltaY * 2.);
    ...
    if ((!fPm->ParameterExists(ParmNameForDivisionMaxStepSize)) && fPm->ParameterExists(GetFullParmName("MaxStepSize")))
        fPm->AddParameter("d:" + ParmNameForDivisionMaxStepSize,
            GetFullParmName("MaxStepSize") + +" " + fPm->GetUnitOfParameter(GetFullParmName("MaxStepSize")));

    G4VSolid*        ZDivisionSolid = new G4Box(divisionName, deltaX, deltaY, deltaZ);
    G4LogicalVolume* ZDivisionLog   = CreateLogicalVolume(divisionName, fPhotoDetectorMaterial, ZDivisionSolid);
    ...
    CreatePhysicalVolume(divisionName, ZDivisionLog, YDivisionPhys, kZAxis, fPtDtDivisionCount[2], deltaZ * 2.);
    ZDivisionLog->SetVisAttributes(GetVisAttributes(""));
    G4LogicalSkinSurface* PtDtSurf2 = new G4LogicalSkinSurface("PtDtSurf2", ZDivisionLog, OpSkinSurface1);

    SetLogicalVolumeToBeSensitive(ZDivisionLog);
}

void FlatImager::GetCrystalGapX(void) {
    fCrystalGapX = fPm->ParameterExists(GetFullParmName("Crystal/HXGap"))
                       ? fPm->GetDoubleParameter(GetFullParmName("Crystal/HXGap"), "Length")
                       : 0 * mm;
}

void FlatImager::GetCrystalGapY(void) {
    fCrystalGapY = fPm->ParameterExists(GetFullParmName("Crystal/HYGap"))
                       ? fPm->GetDoubleParameter(GetFullParmName("Crystal/HYGap"), "Length")
                       : 0 * mm;
}

void FlatImager::GetCrystalGapZ(void) {
    fCrystalGapZ = fPm->ParameterExists(GetFullParmName("Crystal/ZGap"))
                       ? fPm->GetDoubleParameter(GetFullParmName("Crystal/ZGap"), "Length")
                       : 0 * mm;
}

void FlatImager::GetHLX(void) {
    fHLX = fPm->ParameterExists(GetFullParmName("HLX"))
               ? fPm->GetDoubleParameter(GetFullParmName("HLX"), "Length")
               : (fCrystalHLX + fCrystalReflectorThickness + fCrystalGapX) * NbOfXBins * mm;
    if (fHLX < (fCrystalHLX + fCrystalReflectorThickness + fCrystalGapX) * NbOfXBins * mm) {
        G4cerr << "Detector HLX is too small" << G4endl;
    }
}

void FlatImager::GetHLY(void) {
    fHLY = fPm->ParameterExists(GetFullParmName("HLY"))
               ? fPm->GetDoubleParameter(GetFullParmName("HLY"), "Length")
               : (fCrystalHLY + fCrystalReflectorThickness + fCrystalGapY) * NbOfYBins * mm;
    if (fHLY < (fCrystalHLY + fCrystalReflectorThickness + fCrystalGapY) * NbOfYBins * mm) {
        G4cerr << "Detector HLY is too small" << G4endl;
    }
}

void FlatImager::GetHLZ(void) {
    fHLZ = fPm->ParameterExists(GetFullParmName("HLZ"))
               ? fPm->GetDoubleParameter(GetFullParmName("HLZ"), "Length")
               : fPhotoDetectorHLZ + fCrystalHLZ + fTotalPrefilterThicknesses * 0.5 + fCollimatorHL * mm;
    if (fHLZ < fPhotoDetectorHLZ + fCrystalHLZ + fTotalPrefilterThicknesses * 0.5 + fCollimatorHL * mm) {
        G4cerr << "Detector HLZ is shorter than actual length" << G4endl;
    }
}

G4VPhysicalVolume* FlatImager::Construct(void) {
    BeginConstruction();
    G4cout << "Start constructing flat-panel detector " << fName << G4endl;
    ...
    GetHLY();
    GetHLZ();
    G4cout << "HLX: " << fHLX << " HLY: " << fHLY << " HLZ: " << fHLZ << G4endl;
    ...
    if (fCrystalShape == "Box") {
        ...
        if (fCrystalReflectorThickness > 0) {
            fCrystalRadiTotal += fCrystalReflectorThickness;
            ReflectorSolid = new G4Tubs("RfSd", fCrystalRadius, fCrystalRadiTotal, fCrystalHL, 0, 2 * M_PI);
        }
    }
    G4VSolid* EmptyBoxSolid = new G4Box(
        "BxSd", fHLX, fHLY, fPhotoDetectorHLZ + fCrystalHLZ + fTotalPrefilterThicknesses * 0.5 + fCollimatorHL);

    G4LogicalVolume* CrystalDetectorLV = CreateLogicalVolume("CrDtLV", fMaterial, CrystalDetectorSolid);
    ...
    const G4double CollimatorTransX = 0;   //std::sin(Angle) * (0 + fCollimatorHL);
    const G4double CollimatorTransY = 0;   //std::sin(Angle) * (0 + fCollimatorHL);
    const G4double CollimatorTransZ = (fTotalPrefilterThicknesses * 0.5 + fCrystalHLZ + fPhotoDetectorHLZ);

    const G4double CrystalTransX = std::sin(Angle) * (0 + 2 * fCollimatorHL);
    const G4double CrystalTransY = 0;   // std::sin(Angle) * (0 + 2 * fCollimatorHL);
    const G4double CrystalTransZ =
        std::cos(Angle) * (fPhotoDetectorHLZ - fCollimatorHL - fTotalPrefilterThicknesses * 0.5);
    G4ThreeVector* TransVectorCollimator = new G4ThreeVector(CollimatorTransX, CollimatorTransY, CollimatorTransZ);
    ...
    if (fCollimatorExists) {
        CollimatorDetectorSolid = new G4Box("CoDtSd", fHLX, fHLY, fCollimatorHL);
        CollimatorOpeningSolid =
            new G4Box("CoOpSd", Imager::GetCollimatorOpeningHLX(), Imager::GetCollimatorOpeningHLY(), fCollimatorHL);
        CollimatorDetectorLV = CreateLogicalVolume("CoDtLV", fCollimatorMaterial, CollimatorDetectorSolid);
        CollimatorOpeningLV  = CreateLogicalVolume("CoOpLV", fCollimatorOpeningMaterial, CollimatorOpeningSolid);
        CollimatorDetectorPV = CreatePhysicalVolume(
            "CoBxPV", detectorIndex, false, CollimatorDetectorLV, RotMatrix, TransVectorCollimator, fEnvelopePhys);
    ...
    G4VPhysicalVolume* CrystalDetectorPV = CreatePhysicalVolume(
        "CrDtPV", detectorIndex, false, CrystalDetectorLV, RotMatrix, TransVectorCrystal, fEnvelopePhys);
    ...
    if (fCrystalReflectorThickness > 0) {
        G4LogicalSkinSurface* RfSurf = new G4LogicalSkinSurface("RfSurf", ReflectorLV, OpSkinSurface2);
    }

    G4LogicalSkinSurface* ScinSurf = new G4LogicalSkinSurface("ScinSurf", CrystalDetectorLV, OpSkinSurface2);

    for (G4int i = 0; i < NbOfYBins; ++i) {
        const G4double YCenter =
            -fHLY + (2 * i + 1) * (fCrystalHLY + fCrystalReflectorThickness) + fCrystalGapY * (2 * i+1);
        for (G4int j = 0; j < NbOfXBins; ++j) {
            const G4double XCenter =
                -fHLX + (2 * j + 1) * (fCrystalHLX + fCrystalReflectorThickness) + fCrystalGapX * (2 * j+1);
            G4ThreeVector* TransVector = new G4ThreeVector(XCenter, YCenter, 0);
            if (fCollimatorExists) {
                CreatePhysicalVolume(
                    "CoOpBxPV", GetCrystalBoxID(i, j), true, CollimatorOpeningLV, 0, TransVector, CollimatorDetectorPV);
            }
            if (fCrystalReflectorThickness > 0) {
                auto* rftb = CreatePhysicalVolume(
                    "RfPV", i * NbOfYBins + j, true, ReflectorLV, RotMatrix2, TransVector, CrystalDetectorPV);
            }
            auto* crtb = CreatePhysicalVolume(
                "CrPV", i * NbOfYBins + j, true, CrystalLV, RotMatrix2, TransVector, CrystalDetectorPV);
        }
    }

    // delete crystalCounter;
    SetTooComplexForOGLS();
    G4cout << "Flat-panel detector " << fName << " constructed" << G4endl;
