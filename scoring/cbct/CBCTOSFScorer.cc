...
#include <iostream>
// #include <filesystem>
#include "CBCTOSFScorer.hh"
#include "G4PSDirectionFlag.hh"
#include <dirent.h>
#include <glob.h>

G4Mutex CBCTOSFScorer::aMutex = G4MUTEX_INITIALIZER;

CBCTOSFScorer::CBCTOSFScorer(TsParameterManager* pM, TsMaterialManager* mM, TsGeometryManager* gM,
    ...
    fEfficiencyEnergy =
        fPm->GetDoubleVector(GetFullParmName("DetectorEfficiency/Energies"), "Energy");
...
CBCTOSFScorer::~CBCTOSFScorer() {
    // G4cout<<"CBCTOSF here"<<G4endl;
    // for (int kk = 0; kk < kernel_size_u * 2 + 1; kk++)
    // {
    //     free(kernel[kk]);
    // }
...
void CBCTOSFScorer::ReadOSFPath(void) {
...
void CBCTOSFScorer::ReadOSFData(void) {
...
G4bool CBCTOSFScorer::ProcessHits(G4Step* aStep, G4TouchableHistory*) {
    if (!fIsActive) {
        ...
        if (fForcedInteraction) {
            ...
            for (int i = -kernel_size_u; i < kernel_size_u; i++) {
                ...
                for (int j = -kernel_size_v; j < kernel_size_v; j++) {
                    ...
                    AccumulateHit(aStep, kernel[kernel_i][kernel_j] * detection_efficiency, index2);
                    // AccumulateHit(aStep, kernel[kernel_i][kernel_j] * detection_efficiency);
                }
            }

            l.unlock();
            // G4cout << "Index " << index << " Xbin " << xBin << " Ybin " << yBin <<" Zbin " << zBin <<" Index2 "<<index2<< G4endl;
            ...
            if (Rand <= detection_efficiency) {
                ...
                // G4cout << "Index " << index2 << " Xbin " << xBin << " Ybin " << yBin << G4endl;
                // G4cout << "xLast " << xBinNum << " yLast " << yBinNum << G4endl;
                aStep->GetTrack()->SetTrackStatus(fStopAndKill);   // Kill track
