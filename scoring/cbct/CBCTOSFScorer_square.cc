// Scorer for CBCTOSFScorer_square
//
// ********************************************************************
// *                                                                  *
// *                                                                  *
...
#include <glob.h>
#include <dirent.h>
#include "CBCTOSFScorer_square.hh"
#include "G4PSDirectionFlag.hh"

G4Mutex CBCTOSFScorer_square::aMutex = G4MUTEX_INITIALIZER;

CBCTOSFScorer_square::CBCTOSFScorer_square(TsParameterManager *pM, TsMaterialManager *mM, TsGeometryManager *gM, TsScoringManager *scM, TsExtensionManager *eM,
                                           G4String scorerName, G4String quantity, G4String outFileName, G4bool isSubScorer)
    : TsVBinnedScorer(pM, mM, gM, scM, eM, scorerName, quantity, outFileName, isSubScorer)
{
    SetSurfaceScorer();
    SetUnit("");
    fNumberOfEfficiency = fPm->GetVectorLength(GetFullParmName("DetectorEfficiency/Energies"));
    fEfficiencyEnergy = fPm->GetDoubleVector(GetFullParmName("DetectorEfficiency/Energies"), "Energy");
    fEfficiency = fPm->GetUnitlessVector(GetFullParmName("DetectorEfficiency/Efficiency"));
    xBinNum = fPm->GetIntegerParameter(GetFullParmName("XBins"));
    yBinNum = fPm->GetIntegerParameter(GetFullParmName("YBins"));
    kernel_size_u = fPm->GetIntegerParameter(GetFullParmName("HkernelU"));
    kernel_size_v = fPm->GetIntegerParameter(GetFullParmName("HkernelV"));
    E0 = fPm->GetDoubleParameter(GetFullParmName("InitialEnergy"), "Energy") / keV;
    E_step = fPm->GetDoubleParameter(GetFullParmName("EnergyStep"), "Energy") / keV;
    fForcedInteraction = fPm->GetBooleanParameter(GetFullParmName("ForceInteraction"));
    ...
    // ReadNumBins();
    // ReadPixelSpacings();
    std::cout << "kernel size ( " << kernel_size_u << ", " << kernel_size_v << " )" << std::endl;
    ...
    // rkernel = (float **)malloc((kernel_size_u * 2 + 1) * sizeof(float *));
    // for (int kk = 0; kk < kernel_size_u * 2 + 1; kk++)
    // {
    ...
    // }
    srand(0);
    std::cout << "CBCT OSF initialized" << std::endl;
}
CBCTOSFScorer_square::~CBCTOSFScorer_square()
{
    for (int kk = 0; kk < kernel_size_u * 2 + 1; kk++)
    {
        free(kernel[kk]);
...
// void CBCTOSFScorer::UserHookForEndOfRun(){

// }

void CBCTOSFScorer_square::ReadOSFPath(void)
{
    if (fPm->ParameterExists(GetFullParmName("OSFPath")))
    {
        OSFPath = fPm->GetStringParameter(GetFullParmName("OSFPath"));
...
void CBCTOSFScorer_square::ReadOSFData(void)
{
    ...
    for (int ff = 0; ff < files.size(); ff++)
    {
        ...
        for (int row = 0; row < kernel_size_u * 2 + 1; row++)
...
G4bool CBCTOSFScorer_square::ProcessHits(G4Step *aStep, G4TouchableHistory *)
{
    if (!fIsActive)
    {
    ...
    if (fForcedInteraction)
    {
        ...
        for (int i = -kernel_size_u; i < kernel_size_u; i++)
        {
            ...
            for (int j = -kernel_size_v; j < kernel_size_v; j++)
            {
                ...
                (*fEvtMap)[index2] += (kernel[kernel_i][kernel_j] * detection_efficiency) * (kernel[kernel_i][kernel_j] * detection_efficiency);
            }
        }
        ...
        if (Rand <= detection_efficiency)
        {
            ...
            for (int i = -kernel_size_u; i < kernel_size_u; i++)
            {
                ...
                for (int j = -kernel_size_v; j < kernel_size_v; j++)
                {
                    ...
                    kernel_j = j + kernel_size_v;
                    index2 = fComponent->GetIndex(ii, jj, 0);
                    (*fEvtMap)[index2] += kernel[kernel_i][kernel_j] * kernel[kernel_i][kernel_j];
                }
            }
            l.unlock();
