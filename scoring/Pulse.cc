...
Pulse::Pulse(Pulse original, G4double timeShift, G4double energyShift,
             G4ThreeVector positionShift)
    : physicalVolumeName(original.physicalVolumeName),
      energy(original.energy + energyShift),
      endTimeGlobal(timeShift + original.endTimeGlobal),
      endTimeTopas(timeShift + original.endTimeTopas),
      startTimeGlobal(timeShift + original.startTimeGlobal),
      startTimeTopas(timeShift + original.startTimeTopas),
      // unchanged on purpose since the start topas time should stay the same
      // for all events in the same run
      eventStartTimeTopas(original.eventStartTimeTopas),
      position(original.position + positionShift),
      crystalIndex(original.crystalIndex), projectionId(original.projectionId),
      beenDigitized(false), eventID(original.eventID), OriginPosition(original.OriginPosition),
      submoduleIndex(original.submoduleIndex), moduleIndex(original.moduleIndex), detectorIndex(original.detectorIndex),
      historyDepth(original.historyDepth)
{
}

Pulse::Pulse(Pulse first, Pulse second) : physicalVolumeName(first.physicalVolumeName), energy(first.energy + second.energy),
                                          position(first.position), crystalIndex(first.crystalIndex), projectionId(first.projectionId),
                                          beenDigitized(false), eventID(first.eventID), OriginPosition(first.OriginPosition),
                                          submoduleIndex(first.submoduleIndex), moduleIndex(first.moduleIndex), detectorIndex(first.detectorIndex),
                                          historyDepth(first.historyDepth)
{
  ...
  eventStartTimeTopas =
      std::min(second.eventStartTimeTopas, first.eventStartTimeTopas);
}
      ...
      eventID(eventID_)
{
  ...
  G4ThreeVector globalOrigin =
      theTouchable->GetHistory()->GetTopTransform().Inverse().TransformPoint(
          origin);

  position = G4ThreeVector(0., 0., 0.); // = aStep->GetPostStepPoint()->GetPosition(); // position; //theTouchable->GetPosition();
