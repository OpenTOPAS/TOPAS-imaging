...
PETScorer::PETScorer(TsParameterManager* pM, TsMaterialManager* mM, TsGeometryManager* gM,
    ...
    // FWHM of time blur
    fIncludeSourceOrigin = fPm->ParameterExists(GetFullParmName("IncludeSourceOrigin"))
                               ? fPm->GetBooleanParameter(GetFullParmName("IncludeSourceOrigin"))
                               : false;
    ...
    fConsolidateRuns = fPm->ParameterExists(GetFullParmName("RunConsolidationFrequency"));

    fNbRuns  = fPm->GetIntegerParameter("Tf/NumberOfSequentialTimes");
    runCount = 0;
    if (fIncludeCastorOutput) {
        fCastorIncludeTOF = fPm->ParameterExists(GetFullParmName("CastorIncludeTOF"))
    ...
    fMinDistanceInCoincidence =
        fPm->GetDoubleParameter(GetFullParmName("MinDistanceInCoincidence"), "Length");
    GetDigitizers();
    G4cout << "The number of Digitizers " << fNbDigitizers << G4endl;
    G4AutoLock l(&aMutex);
    if (digitizers->empty()) {
        for (G4int digi_ind = 0; digi_ind < fNbDigitizers; digi_ind++) {
            if (fDigitizerList[digi_ind] == "EnergyCutoff") {
                G4cout << "Registering digitizer: " << fDigitizerList[digi_ind] << G4endl;
                fMinEnergy = fPm->GetDoubleParameter(
                    GetFullParmName("Digitizer/EnergyCutoff/Threshold"), "Energy");
                fMaxEnergy = fPm->GetDoubleParameter(
                    GetFullParmName("Digitizer/EnergyCutoff/Uphold"), "Energy");
            ...
            } else if (fDigitizerList[digi_ind] == "TimeResolution") {
                fTimeResolution =
                    fPm->GetDoubleParameter(GetFullParmName("Digitizer/TimeResolution"), "Time");
                G4cout << "Registering digitizer: " << fDigitizerList[digi_ind] << G4endl;
                G4cout << "Time resolution: " << fTimeResolution << G4endl;
                digitizers->push_back(new TimeResolution(fTimeResolution));
            } else if (fDigitizerList[digi_ind] == "DeadTime") {
                G4cout << "Registering digitizer: " << fDigitizerList[digi_ind] << G4endl;
                fDeadTime = fPm->GetDoubleParameter(GetFullParmName("Digitizer/DeadTime"), "Time");
                G4cout << "Dead time: " << fDeadTime << G4endl;
                digitizers->push_back(new DeadTime(fDeadTime));
            } else if (fDigitizerList[digi_ind] == "PulsePileUp") {
                fPileupTimingWindow = fPm->GetDoubleParameter(
                    GetFullParmName("Digitizer/PulsePileUp/TimingWindow"), "Time");
                G4cout << "Registering digitizer: " << fDigitizerList[digi_ind] << G4endl;
                G4cout << "Pulse pile-up timinging window: " << fDeadTime << G4endl;
                digitizers->push_back(new PileUpModule(fPileupTimingWindow));
            } else if (fDigitizerList[digi_ind] == "SigmoidalThresholder") {
                fSigmoidalThreshold = fPm->GetDoubleParameter(
                    GetFullParmName("Digitizer/SigmoidalThresholder/Threshold"), "Energy");
                fSigmoidalAlpha = fPm->GetUnitlessParameter(
                    GetFullParmName("Digitizer/SigmoidalThresholder/Alpha"));
                fSigmoidalPercent = fPm->GetUnitlessParameter(
                    GetFullParmName("Digitizer/SigmoidalThresholder/Percent"));
                G4cout << "Registering digitizer: " << fDigitizerList[digi_ind] << G4endl;
                G4cout << "Energy: " << fDeadTime << " Alpha: " << fSigmoidalAlpha << " Percent"
                       << fSigmoidalPercent << G4endl;
                digitizers->push_back(new SigmoidalThresholder(
                    fSigmoidalThreshold, fSigmoidalAlpha, fSigmoidalPercent));
            }
        }
        // digitizers->push_back(new EnergyCutoffModule(fMinEnergy, fMaxEnergy));
        // digitizers->push_back(new TimeResolution(fTimeResolution));
    ...
    l.unlock();
    coincidenceCounterPrompts = new CoincidenceCounterDistance(fMinDistanceInCoincidence);

    coincidenceCounterRandoms = new CoincidenceCounterDistance(fMinDistanceInCoincidence);
...
void PETScorer::GetDigitizers(void) {
    fNbDigitizers = fPm->ParameterExists(GetFullParmName("Digitizers"))
                        ? fPm->GetVectorLength(GetFullParmName("Digitizers"))
                        : 0;
    if (fNbDigitizers > 0) {
        fDigitizerList = fPm->GetStringVector(GetFullParmName("Digitizers"));
    }
}
void PETScorer::InitializeOutputColumns() {
...
void PETScorer::FillOutputColumns(std::vector<Pulse>* pulsesSoFar) {
    ...
    for (auto coincidence : fCoincidencesPrompts) {
        ...
        fTime1       = pulse1.eventStartTimeTopas;
        fTime2       = pulse2.eventStartTimeTopas;
        minTime      = std::min(minTime, std::min(fTime2, fTime1));
        maxTime      = std::max(maxTime, std::max(maxTime, fTime1));
    }

    fminTime = std::min(minTime, fminTime);
    fmaxTime = std::max(maxTime, fmaxTime);

    G4double randomIntensityRate =
        fCoincidencesRandoms.size() / (maxTime - minTime);   //#counts/time
    ...
    for (auto coincidence : fCoincidencesPrompts) {
        ...
        if (fIncludeCastorOutput) {
            ...
            static_assert(sizeof(float) == 4, "The float type expected to have 32 bits");

            // Might not work properly on all platforms
            foutputFile.write(reinterpret_cast<char*>(&fTimeCastor), sizeof(uint32_t));
            if (fCastorIncludeRandomsCorrection) {
                foutputFile.write(
                    reinterpret_cast<char*>(&fUnnormalizedRandomIntensityCastor), sizeof(float));
            }

            if (fCastorIncludeTOF) {
                foutputFile.write(reinterpret_cast<char*>(&fTimeDeltaCastor), sizeof(float));
            }
            foutputFile.write(reinterpret_cast<char*>(&fCrystalID1Castor), sizeof(uint32_t));
            foutputFile.write(reinterpret_cast<char*>(&fCrystalID2Castor), sizeof(uint32_t));
        }
...
void PETScorer::Output() {
    DigitizerScorer::Output();
    ...
    if (fIncludeCastorOutput) {
        ...
        if (fCastorIncludeTOF) {
            ...
            header << "List TOF measurement range (ps): " << (fmaxTimeDelta - fminTimeDelta) / ps
                   << std::endl;
        }
        header.close();
    }
...
void PETScorer::CoincidenceDetection(std::vector<Pulse>* singles,
    std::vector<Coincidence>& coincidencesPrompts, std::vector<Coincidence>& coincidencesRandoms,
    G4bool onlyDoStart, G4bool garbageCollect) {
    ...
    G4double limitedTimeWindowMarginLength = 2 * fCoincidenceTimeDelayRandoms +
                                             2 * fCoincidenceTimeWindowRandoms +
                                             2 * fCoincidenceTimeWindowPrompts;
